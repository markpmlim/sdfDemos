...
class MetalView: MTKView, NSWindowDelegate {
    ...
    // The instantiated texture object is from a 2:1 equirectangular map.
    func createCubemapTexture(_ fileName: String) {
        let options: [String : NSObject] = [
            MTKTextureLoaderOptionTextureUsage : NSNumber(value: MTLTextureUsage.shaderRead.rawValue),
            MTKTextureLoaderOptionTextureStorageMode: NSNumber(value: MTLStorageMode.private.rawValue)
        ]
        let textureLoader = MTKTextureLoader(device: self.device!)

        do {
            try inputTexture = textureLoader.newTexture(withName: fileName,
                                                        scaleFactor: 1.0,
                                                        bundle: nil,
                                                        options: options)
        }
        catch {
            fatalError("Can't instantiate the equirectangular texture ")
        }
    }
